...
class colors:
  ...
  # Aquí es donde Álvaro la cagó pero bien poniendo un salto de línea porque le apeteció y encima me echó la culpa a mí
  RESET = "\x1b[0m"
  RED = "\x1b[31m"
  GREEN = "\x1b[32m"
  YELLOW = "\x1b[33m"
...
def introducir_palabra():
  ...
  if len(palabra_usuario) < 5 or len(palabra_usuario) > 5:
    print(f"{colors.RED}La palabra debe contener 5 letras y sin tildes{colors.RESET}")
...
def comprobar_caracteres(palabra_usuario_lista, palabra_lista):
  ...
  comprobacion = ["x", "x", "x", "x", "x"] # Lista de caracteres donde vamos a guardar el resultado de la comprobación
  ...
  for i in range(5):
    ...
    if palabra_usuario_lista[i] == palabra_lista[i]:
      comprobacion[i] = f"{colors.GREEN}{palabra_usuario_lista[i]}{colors.RESET}"
      aciertos += 1
  ...
  for i in range(5):
    ...
    if palabra_usuario_lista[i] in palabra_lista and contador[palabra_usuario_lista[i]] > 0 and palabra_lista[i] != palabra_usuario_lista[i]:
      comprobacion[i] = f"{colors.YELLOW}{palabra_usuario_lista[i]}{colors.RESET}"
      contador[palabra_usuario_lista[i]] -= 1
